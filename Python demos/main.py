...
for i in range(p):
    if Final_components_csize[Final_components[i]] == 1:
        Theta_ii = 1
        for j in G_thresholded.neighbors(i):  # Assuming neighbors is a function returning neighbors of node i in graph G_res
...
print(objective_function(out_FPN["X_est"], S_lambda)["value"])
